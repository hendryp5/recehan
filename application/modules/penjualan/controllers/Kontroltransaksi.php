...
class Kontroltransaksi extends CI_Controller {
	/**
	 * code by rifqie rusyadi
	 * email rifqie.rusyadi@gmail.com
	 */
	public $folder = 'penjualan/kontroltransaksi/';
	
	public function __construct()
	{
		parent::__construct();
		$this->load->model('Kontroltransaksi_m', 'data');
	...
	{
		$data['judul'] 		= 'Kontrol Transaksi';
		$data['head'] 		= 'KPR';
		$data['head2'] 		= 'Cash Bertahap';
		$data['head3'] 		= 'Cash keras';
    ...
    public function ajax_list_kpr()
    {
        $record	= $this->data->get_datatables_kpr();
        $data 	= array();
        $no 	= $_POST['start'];
        ...
        $output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->data->count_all_kpr(),
			"recordsFiltered" => $this->data->count_filtered_kpr(),
			"data" => $data,
		);
		echo json_encode($output);
    }

    public function ajax_list_bertahap()
    {
        $record	= $this->data->get_datatables_bertahap();
        $data 	= array();
        $no 	= $_POST['start'];
        ...
        foreach ($record as $row) {
            ...
            $no++;
            $col = array();
            $col[] = nama_kavling($row->kavling_id);$col[] = $row->nama;
			$col[] = $row->telpon1;
			$col[] = $row->nama;  						
			$col[] = $row->nama;
			$col[] = $row->nama;
        ...
        $output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->data->count_all_bertahap(),
			"recordsFiltered" => $this->data->count_filtered_bertahap(),
			"data" => $data,
		);
		echo json_encode($output);
    }

    public function ajax_list_keras()
    {
        $record	= $this->data->get_datatables_keras();
        $data 	= array();
        $no 	= $_POST['start'];
        ...
        $output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->data->count_all_keras(),
			"recordsFiltered" => $this->data->count_filtered_keras(),
			"data" => $data,
		);
		echo json_encode($output);
    }

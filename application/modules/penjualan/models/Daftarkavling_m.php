...
class Daftarkavling_m extends MY_Model
{
	...
	//ajax datatable
    public $column_order = array('id','kavling','type','tanah','bangunan','status','harga','perumahan_id',null); //set kolom field database pada datatable secara berurutan
    public $column_search = array('kavling','type','tanah','bangunan','status','harga','perumahan_id'); //set kolom field database pada datatable untuk pencarian
    ...
    private function _get_datatables_query()
    {
        ...
        {
            $this->db->where('perumahan_id', $this->input->post('perumahan_id'));
        }
    ...
    public function get_group1()
    {
        ...
        if($query->num_rows() > 0){
        $dropdown[''] = 'Pilih Tipe Unit';
        foreach ($query->result() as $row)
        {
            $dropdown[$row->type] = $row->type;
        }
        }else{
            $dropdown[''] = 'Belum Ada Unit Tersedia'; 
        }
